...
class UserController
{
    ...
    public function create()
    {
        ...
        if (isset($data->nome) && isset($data->matricula) && isset($data->data_nasc)) {
            try {
                $this->user->create($data->nome, $data->matricula, $data->data_nasc);
    ...
    {
        $data = json_decode(file_get_contents("php://input"));
        if (isset($id) && isset($data->nome) && isset($data->matricula) && isset($data->data_nasc) ) {
            try {
                $count = $this->user->update($id, $data->nome, $data->matricula, $data->data_nasc);
                if ($count > 0) {
